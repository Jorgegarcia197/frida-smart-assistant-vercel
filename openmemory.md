...
### Token / context saving (telemetry, trim, retry, compact)

- **Phase 1–2:** `.env.example` documents Frida + gateway (`openai-ai-sdk`) compaction flags. `DEBUG_CHAT_CONTEXT_SIZE=true` logs `estimateChatContextFromUiMessages` (histogram + char / ÷4 token guess) after file expansion / microcompact in `app/(chat)/api/chat/route.ts` (`lib/ai/chat-context-estimate.ts`). The composer **context ring** prefers `data-context-payload-estimate` from the chat stream (same numbers as that log for the in-flight user turn, keyed by `triggerMessageId`); when idle it re-estimates with client-side `trimUiMessagesForModel` using trim fields from the last payload (file expansion remains server-only). Hover rows still show **provider** usage from message metadata.
- **Phase 3:** `CHAT_MICROCOMPACT_ENABLED=true` runs `trimUiMessagesForModel` from `lib/ai/trim-ui-messages-for-model.ts` after `expandFilePartsForModel` (DB messages unchanged). Default substring allowlist stubs heavy tools including `web_search` / `web_fetch` plus codebase/file-style names. Optional `CHAT_MICROCOMPACT_TOOL_SUBSTRINGS` CSV overrides; `CHAT_MICROCOMPACT_KEEP_MESSAGES` guards recent tail.
- **Phase 4:** `CHAT_RETRY_ON_CONTEXT_OVERFLOW=true` switches the UI stream path to `drainUiMessageStreamToWriter` (`lib/ai/merge-ui-message-stream.ts`) so read errors propagate; one retry uses `trimUiMessagesForModel(..., { preset: 'overflow-retry' })` and emits transient `data-context-trim-notice` (toast in `components/chat.tsx`). Detection: `isContextOverflowError` in `lib/ai/is-context-overflow-error.ts`.
- **Phase 5:** `ENABLE_MANUAL_THREAD_COMPACT=true` enables `POST /api/chat/compact` (`app/(chat)/api/chat/compact/route.ts`): summarizes older turns with `generateText`, `deleteMessagesByChatIdWhereCreatedAtBefore` (`lib/db/queries.ts`), inserts a synthetic assistant “Thread compacted” row ordered before the kept tail.
- **Phase 6:** `SYSTEM_PROMPT_DENSITY=compact` caps MCP tool bullets in `buildSystemPromptSections` (`lib/ai/prompts.ts`).
...
### Anthropic web + server tool passthrough (`lib/ai/tools/anthropic-web-tools.ts`, `lib/ai/web-tool-result-capture.ts`, `openai-ai-sdk/src/ai_sdk/providers/anthropic.py`)

- For Claude models the chat route registers `web_search`, `web_fetch`, `$BUILT_IN_WEB_SEARCH` as `dynamicTool` passthroughs via `createAnthropicWebPassthroughTools` (alongside `createAnthropicSkillsPassthroughTools` for `text_editor_code_execution` / `bash_code_execution` / `code_execution`). Anthropic executes server-side; local `execute` mirrors results to the AI SDK, but **web tools** normalize before return (see below).
- `passthroughExecute` resolves the result in this order: (1) `takeProviderWebToolResult(options.toolCallId)` — an `AsyncLocalStorage`-backed map filled by `createSseWebToolTapTransform` when it sees `_result` merged into `function.arguments` on the SSE wire; (2) `extractUpstreamWebPayload(args)` — reads `args._result` when the AI SDK parsed a `_result`-bearing args JSON; (3) `{delegated:true, note, args}` stub (treated as empty by the model — triggers backtracking / apologies on multi-step turns). Steps (1)–(2) are passed through `normalizeAnthropicWebToolResult` (`lib/ai/normalize-anthropic-web-tool-result.ts`): redacts `encrypted_content` / long base64-like runs, caps markdown `summary` at 100k chars, extracts a bounded `sources` list (URLs + optional titles), puts provider repr **Details** inside a **fenced code block** in `summary` (so Streamdown does not autolink URLs inside a `<p>` and break hydration), and adds `headlineMarkdown` + URL-stripped `detailsPlain` for UI. The tool returns `{ _fridaWebNormalized: true, summary, sources, headlineMarkdown?, detailsPlain? }`; `toModelOutput` sends **only** `summary` as text to the model. `components/tool-card.tsx` (delegated web tools) renders **Sources** (+ optional `InlineCitation` carousel) first, a short `Response` for `headlineMarkdown` only, and a monospace `<pre>` for `detailsPlain`; legacy rows without `headlineMarkdown` still use full `Response` on `summary`. Debug args JSON is redacted for the same patterns.
- `activeTools` deliberately omits `$BUILT_IN_WEB_SEARCH` alongside `web_search` so the model doesn't invoke both aliases in one turn (upstream would then run the search twice). It stays registered in `toolsForModel` for history replay / gateway aliasing.
- Assistant reply UI: `sendSources: true` emits `source-url` parts; `components/message-sources.tsx` uses one `SourcesContent` with `WebSourceUrlRows` (favicon + title + hostname). Bracket refs `[1]`… in assistant text map to `InlineCitation` via `components/assistant-bracket-citations.tsx` when order matches sources; `InlineCitationCardTrigger` supports optional `label` (page title). Web tools prompt section asks for bracket cites in the final answer.
- **Double-emission pitfall (fixed upstream):** `openai-ai-sdk` previously yielded each `server_tool_use` as `__tool_calls__` twice — once at `content_block_stop` (args without `_result`), then again after `stream.get_final_message()` (args with `_result` merged). `_stream_standard` serialises each yield into OpenAI SSE deltas at the same `index`, and `@ai-sdk/openai-compatible` finalises a tool call the first time `isParsableJson(function.arguments)` returns true (`hasFinished=true`) and `continue`s on later deltas at that index. Net effect: `execute()` ran with the `_result`-less args and returned the `delegated:true` stub. Fix: in `AnthropicModel.stream_text`, drop the provisional emit (keep `pending_server_tool` tracking but never yield at `content_block_start` no-idx branch or at `content_block_stop`); the post-`final_message` loop is the single source of truth and emits args with `_result` merged, letting the adapter's first parse surface the real payload. The TS-side SSE tap (`lib/ai/web-tool-result-capture.ts`) stays as a safety net for non-stream JSON responses and future edge cases but cannot by itself defeat the `hasFinished=true` race.
