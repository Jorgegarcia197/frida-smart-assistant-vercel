...
function PureMcpToolCard({
  ...
  const displayContent = state === 'result' ? formatContent(result) : formatContent(args);
  const isLoading = state === 'call';
  
  // Check if result contains an error
  const hasError = state === 'result' && result && (
    typeof result === 'object' && (
      result.error || 
      result.isError || 
      (result.content && Array.isArray(result.content) && result.content.some((c: any) => c.type === 'text' && c.text && c.text.toLowerCase().includes('error'))
    )
  ));
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-sm font-medium">
              {isLoading ? `Calling MCP tool: ${actualToolName}...` : 
               hasError ? `${actualToolName} Error` : 
               `MCP tool "${actualToolName}" was executed`}
            </CardTitle>
          </div>
